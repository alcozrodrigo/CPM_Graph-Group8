    ...
    private func printProject(activities: [Activity], criticalActivities: [Activity]) {
        ...
        for activity in activities {
            ...
            if (activity.actualFinish != 0) {
                ...
                result.append("ID = \(activity.id): \(activity.name)\n" ).bold()
                result.append("    Actual Start").bold()
                result.append(": \(activity.actualStart)\n")
                
                result.append("    Early Finish").bold()
                result.append(": \(activity.earlyFinish)\n")
                
                result.append("    Late Finish").bold()
                result.append(": \(activity.lateFinish)\n")
                
                result.append("    Total Float").bold()
                result.append(": \(activity.totalFloat)\n")
                
                result.append("    Free Float").bold()
                result.append(": \(activity.freeFloat)\n")
            } else {
                result.append("ID = \(activity.id): \(activity.name)\n" ).bold()
                
                result.append("    Early Start").bold()
                result.append(": \(activity.earlyStart)\n")
                
                result.append("    Early Finish").bold()
                result.append(": \(activity.earlyFinish)\n")
                
                result.append("    Late Start").bold()
                result.append(": \(activity.lateStart)\n")
